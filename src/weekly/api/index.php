...
function createWeek(PDO $db, array $data): void
{
    ...
    if (
        !isset($data['title'], $data['start_date']) ||
        trim($data['title']) === '' ||
    ...
    if (isset($data['links']) && is_array($data['links'])) {
        $links = array_values(array_filter(array_map('trim', $data['links']), function ($link) {
            return $link !== '';
        }));
    }
...
function createComment(PDO $db, array $data): void
{
    ...
    if (
        !isset($data['week_id'], $data['author'], $data['text']) ||
        trim((string)$data['week_id']) === '' ||
        trim($data['author']) === '' ||
        trim($data['text']) === ''
    ) {
